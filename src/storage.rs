...
impl Storage {
    ...
    async fn prepare(&self) -> anyhow::Result<()> {
        ...
        sqlx::query(
            "CREATE INDEX IF NOT EXISTS idx_location_logs_station_id ON location_logs (station_id);",
        )
        .execute(pool)
        .await?;

        sqlx::query(
            "CREATE INDEX IF NOT EXISTS idx_location_logs_line_id ON location_logs (line_id);",
        )
        .execute(pool)
        .await?;
