...
WebSocket を購読できるのは**観測用トークンだけ**です。イベント用・遠隔測定用トークンでは接続がハンドシェイク時に HTTP 401 で拒否されます。逆に観測用トークンでは GraphQL Mutation を実行できないため、閲覧用クライアントに配るトークンとして安全に分離されています(観測用トークンは GraphQL の履歴取得 Query `logEvents` / `interactionEvents` / `locations` にも使えます。[react-tanstack-query.md](./react-tanstack-query.md) を参照)。

> **公開ブラウザビルドに観測用トークンを直接埋め込まないでください。** 観測用トークンは全利用者が共有する秘密であり、ビルド成果物に含めればページを開いた誰でもそれを取り出して `location_update` / `log` / `interaction` を購読できてしまいます。不特定多数に公開するクライアントでは、ブラウザにトークンを渡さず、BFF(Backend for Frontend)や認可プロキシ経由で利用者ごとの認可・必要イベントの絞り込みを行ってください。社内ツールなど配布先を厳密に管理できる非公開デプロイに限り、埋め込みは許容されますが、その場合も漏えい時に速やかにローテーションできる運用にしておいてください。
...
- リングバッファの件数はサーバー側の `ring_size`(デフォルト 1000)で決まります。接続時のスナップショットでそれ以上さかのぼる必要がある場合は、同じ観測用トークンで GraphQL の履歴取得 Query(`logEvents` / `interactionEvents` / `locations`)を利用してください([react-tanstack-query.md](./react-tanstack-query.md) を参照)。集計データだけでよければ認証不要の `accuracyByLine` もあります。
